...
class NLPUtils:
    ...
    def summarize_text(
        ...
        """
        inputs = bart_tokenizer(text, return_tensors="pt", max_length=model_max_tokens, truncation=True)
    ...
    def summarize_windows(
        ...
        windows: List[str], 
        model_max_tokens: int, 
        gen_kwargs: Optional[Dict] = None) -> List[str]:
        """
        Batch summarize windows using the BART model.
        """
        out = []
        for w in windows:
            out.append(NLPUtils.summarize_text(bart_model, bart_tokenizer, device, w, model_max_tokens, gen_kwargs))
        return out
    ...
    def merge_and_polish(
        ...
        sources = "\n\n".join(f"[{i+1}] {s.strip()}" for i, s in enumerate(chunk_summaries) if s and s.strip())
        ...
        encoder_cap = min(model_max_tokens, getattr(bart_tokenizer, "model_max_length", 1024) or 1024)
        ...
        if avail < 64:
            ...
            header_ids = bart_tokenizer(header, add_special_tokens=True, return_tensors="pt")["input_ids"][0]
    ...
    def merge_and_polish_1(
        ...
        return NLPUtils.summarize_text(bart_model, bart_tokenizer, device, prompt, model_max_tokens, gen_kwargs)
    
    @staticmethod
    def run_quality_gates(summary: str, must_keep: List[str], source_entities: Dict[str, List[str]]) -> Dict[str, object]:
        checks = {"entity_coverage": True, "temporal": True, "numeric": True, "redundancy_ok": True, "missing_entities": []}
        ...
        # temporal anchoring
        if not re.search(r"\b(\d{4}-\d{2}-\d{2}|\d{1,2}[./-]\d{1,2}[./-]\d{2,4}|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s+\d{1,2},?\s+\d{2,4})\b", summary, re.I):
    ...
    def extract_entities(text: str,lang: str="en") -> Dict[str, List[str]]:
        entities = {"PERSON": [], "ORG": [], "GPE": [], "LOC": [], "DATE": [], "CARDINAL": []}
        added = False
        ...
        if not added:
            try:
                ...
                for item in ner(text): # cap for speed
                    ...
                    if label and val:
                        key = "ORG" if "ORG" in label else (
                            "PERSON" if "PER" in label else (
                            "GPE" if "LOC" in label else ""))
                        if key:
        ...
        date_pat = re.compile(r"\b(?:\d{1,2}[./-]\d{1,2}[./-]\d{2,4}|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s+\d{1,2},?\s+\d{2,4}|\d{4}-\d{2}-\d{2})\b", re.I)
    ...
    @staticmethod
    def build_must_keep_entities(entities: Dict[str, List[str]], top_n: int = 15) -> List[str]:
        flat = []
    ...
    def compress_news_adaptive(
        ...
        keep_fraction = lo + (hi - lo) * (0.3 + 0.7 * (0.4 - min(0.4, redundancy))) # map
        ...
        # TF-IDF over sentences
        vec = TfidfVectorizer(ngram_range=(1, 2), sublinear_tf=True, lowercase=True, max_df=0.9)
        #X = vec.fit_transform(sentences)
        ...
        pos = list(reversed([0.85 + 0.15 * (i / max(1, len(sentences) - 1)) for i in range(len(sentences))]))
        raw = 0.75 * base + 0.10 * NLPUtils.np_array(length_norm) + 0.10 * NLPUtils.np_array(has_num) + 0.05 * NLPUtils.np_array(pos)
        ...
        prefix_tokens = len(bart_tokenizer.tokenize(prompt_prefix)) if prompt_prefix else 0
    ...
    def chunk_by_tokens_overlap(
        ...
        while i < len(sentences):
            ...
            if cur_len + tl <= window_tokens:
                ...
                if cur:
                    chunk_text = (prompt_prefix + " " if prompt_prefix else "") + " ".join(cur)
                    ...
                    while j >= 0 and back_len < overlap_tokens:
                        ...
                        j -= 1
                    cur = back
                    cur_len = len(bart_tokenizer.tokenize(" ".join(cur))) + len(bart_tokenizer.tokenize(prompt_prefix))
                else:
        ...
        if cur:
            chunks.append((prompt_prefix + " " if prompt_prefix else "") + " ".join(cur))
        return chunks
    ...
    def np_array(x, dtype=float):
        try:        
            ...
            return type("_A", (), {"__iter__": lambda self: iter(x), "min": lambda self: min(x), "max": lambda self: max(x)})
