...
class PerformanceMonitor:
    ...
    def __init__(self, update_interval: float = 1.0):
        ...
        self.historical_metrics: deque = deque(maxlen=1000)  # Keep last 1000 measurements
        ...
        logger.info(f"performance_monitor.py:PerformanceMonitor initialized with {update_interval}s update interval")
    
    async def start_monitoring(self):
        """Start real-time performance monitoring."""
        if self._is_monitoring:
            logger.warning("performance_monitor.py:Performance monitoring already started")
            return
        ...
        try:
            with self._lock:
                ...
                if uptime > 0:
                    ...
                    self.metrics.batches_per_second = self.metrics.total_batches / uptime
                
                if self.metrics.total_batches > 0:
                    self.metrics.avg_batch_time = self.metrics.total_processing_time / self.metrics.total_batches
                ...
                if self.metrics.total_batches > 0:
                    self.metrics.batch_efficiency = min(1.0, self.metrics.requests_per_second / (self.metrics.total_batches * 10))
    ...
    async def _analyze_performance(self):
        ...
        try:
            ...
            if self.metrics.gpu_usage is not None:
                if self.metrics.gpu_usage < self.performance_thresholds['gpu_utilization_min']:
            ...
            if self.metrics.cpu_usage > self.performance_thresholds['cpu_utilization_max'] * 100:
                recommendations.append({
            ...
            if self.metrics.batch_efficiency < self.performance_thresholds['batch_efficiency_min']:
                recommendations.append({
            ...
            if recommendations:
                for rec in recommendations:
                    ...
                    log_method(f"performance_monitor.py:{rec['type'].upper()}: {rec['message']} - {rec['suggestion']}")
    ...
    def _store_historical_metrics(self):
        ...
        try:
            ...
            logger.error(f"performance_monitor.py:Error storing historical metrics: {e}")
    ...
    def update_concurrency_metrics(self, active_workers: int, queue_depth: int, concurrent_batches: int):
        ...
        try:
            ...
            logger.error(f"performance_monitor.py:Error updating concurrency metrics: {e}")
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        try:
            with self._lock:
                ...
                recent_metrics = list(self.historical_metrics)[-10:] if len(self.historical_metrics) >= 10 else []
                ...
                if len(recent_metrics) >= 2:
                    ...
                    trend_analysis = {
                        'requests_per_second_trend': (last.requests_per_second - first.requests_per_second) / max(first.requests_per_second, 1),
                        'error_rate_trend': (last.error_rate - first.error_rate) / max(first.error_rate, 0.001),
                        'gpu_utilization_trend': (last.gpu_utilization - first.gpu_utilization) if first.gpu_utilization and last.gpu_utilization else 0,
                        'cpu_utilization_trend': (last.cpu_utilization - first.cpu_utilization) / max(first.cpu_utilization, 0.001)
                    }
            ...
            logger.error(f"performance_monitor.py:Error getting performance summary: {e}")
            return {}
    ...
    def _calculate_performance_score(self) -> float:
        ...
        try:
            ...
            if self.metrics.gpu_usage is not None and self.metrics.gpu_usage < 0.7:
                score -= (0.7 - self.metrics.gpu_usage) * 50  # Up to 35 points for GPU underutilization
            
            if self.metrics.cpu_usage > 80:
                score -= (self.metrics.cpu_usage - 80) * 0.5  # Up to 10 points for CPU overutilization
            
            if self.metrics.error_rate > 0.05:
                score -= self.metrics.error_rate * 200  # Up to 10 points for high error rate
            
            if self.metrics.batch_efficiency < 0.8:
                score -= (0.8 - self.metrics.batch_efficiency) * 25  # Up to 20 points for low batch efficiency
            
            return max(0.0, min(100.0, score))
            
        except Exception as e:
            logger.error(f"performance_monitor.py:Error calculating performance score: {e}")
    ...
    def _generate_optimization_recommendations(self) -> List[Dict[str, str]]:
        ...
        try:
            ...
            logger.error(f"performance_monitor.py:Error generating optimization recommendations: {e}")
    ...
    async def force_garbage_collection(self):
        ...
        try:
            ...
            logger.info(f"performance_monitor.py:Forced garbage collection, collected {collected} objects")
        except Exception as e:
