...
class VectorEmbeddingModelManager(AbstractModel[SentenceTransformer]):
    ...
    def __init__(self, settings: Optional[Any] = None):
        ...
        self._embedding_dimension = (
            768  # all-mpnet-base-v2 produces 768-dimensional embeddings
        )
    ...
    def _load_tokenizer(self) -> Optional[Any]:
        ...
        try:
            ...
            self.logger.debug(
                f"Loaded tokenizer for {self._model_name} "
    ...
    def encode_texts(
        ...
        if instance is None:
            with self.acquire() as instance:
                return self._perform_encoding(
                    texts, instance, batch_size, convert_to_tensor
                )
        else:
            return self._perform_encoding(
                texts, instance, batch_size, convert_to_tensor
    ...
    def encode_text(
        self, text: str, instance: Optional[ModelInstance[SentenceTransformer]] = None
    ) -> List[float]:
        """
    ...
    def _compute_similarity(
        self, text1: str, text2: str, instance: ModelInstance[SentenceTransformer]
    ) -> float:
