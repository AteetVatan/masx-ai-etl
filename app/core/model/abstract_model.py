...
class AbstractModel(ABC, Generic[T]):
        ...
        try:
            ...
            if gpu_memory_info:
                ...
                if model_vram_estimate > 0:
                    ...
                    pool_size = min(max_instances, self.settings.model_pool_max_instances)
                    self.logger.info(f"GPU pool size calculated: {pool_size} (VRAM: {free_vram_bytes // 1024**3:.1f}GB, model estimate: {model_vram_estimate // 1024**3:.1f}GB)")
                    return pool_size
        except Exception as e:
    ...
    def _get_model_vram_estimate(self) -> int:
        ...
        estimates = {
            ...
            'translation': int(3.2 * 1024**3),  # ~3.2 GB measured for NLLB-200-distilled-600M
        }
    ...
    def initialize(self) -> None:
        ...
        self.logger.info(f"Initialized {self.model_type} manager with pool size {pool_size}")
    ...
    @asynccontextmanager
    async def acquire(self, timeout: Optional[float] = None, destroy_after_use: bool = False):
        """
    ...
    async def release_instance(self, instance: ModelInstance[T], destroy: bool = False) -> None:
        """
    ...
    def release_instance_sync(self, instance: ModelInstance[T], destroy: bool = False) -> None:
        """
        Release a model instance back to the pool.
    ...
    def cleanup(self) -> None:
        ...
        if self._pool:
            ...
            self.shrink_pool(0)
            self.logger.info(f"Cleaned up {self.model_type} manager, destroyed {stats['total']} instances")
