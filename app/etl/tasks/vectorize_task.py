...
class VectorizeTask:
    ...
    async def run(self, feeds: List[FeedModel]) -> str:
        ...
        try:
            ...
            embedding_manager: VectorEmbeddingModelManager = self.inference_runtime.model_manager
            ...
            for i in range(0, len(feeds), batch_size):
                ...
                self._logger.info(f"VectorizeTask: processing batch of {len(batch)} feeds")
    ...
    async def _initialize_inference_runtime(self):
        ...
        try:
            ...
            self._logger.info("VectorizeTask: Inference runtime initialized for embedding generation")

        except Exception as e:
            self._logger.error(f"VectorizeTask: Failed to initialize inference runtime: {e}")
            raise
    ...
    async def _process_batch(self, feeds: List[FeedModel]) -> List[FeedModel]:
        ...
        try:
            ...
            async def run(feed: FeedModel) -> FeedModel | None:
                ...
                async with embedding_manager.acquire(destroy_after_use=False) as instance:
                    try:
                        # Generate embedding for the feed text
                        text = feed.processed_text.strip()
                        
                        # Encode text into tokens
                        encoded = instance.tokenizer.encode(text, add_special_tokens=True)

                        print("Number of tokens:", len(encoded))
                        print("Max tokens allowed:", instance.tokenizer.model_max_length)

                        if not text:
                            return None
                            
                        embedding = await self.cpu_executors.run_in_thread(
                            embedding_manager.encode_text,
                            text,
                            instance
                        )
                        
                        if embedding is None:
                            raise Exception("VectorizeTask: Error generating embedding for feed")
    ...
    async def _store_vectorized_feeds(self, feeds: List[FeedModel]) -> None:
        ...
        try:
            ...
            for feed in feeds:
                if not hasattr(feed, "embedding") or feed.embedding is None or len(feed.embedding) == 0:
                    continue
