...
from ...nlp.nllbtranslator_multiple import get_nllb_translator


class Translator_old:
    """
    Provides utilities to normalize and translate multilingual text to English.
    Uses NLLB translator from the model pool for efficient resource management.
    """
    ...
    def __init__(self, max_chars=2000, retries=3, delay=2):
        ...
        # Get NLLB translator from the model pool
        self.nllb_translator = get_nllb_translator()
        
        # Google translator for fallback
    ...
    def ensure_english_sync(self, text: str) -> str:
        ...
        try:
            ...
            return Translator_old._run_in_thread(self.ensure_english(text))
    ...
    async def nllb_translate(
        ...
        """
        Translate the text using NLLB from the model pool.
        """
        try:
            # Split the text into chunks for efficient processing
            self.logger.info(
                f"translator.py:[Translation] Using NLLB: {source_lang} → {target_lang}"
            )
            chunks = self.__split_text_smart(text, 400)
            
            # Use the NLLB translator from the model pool
