...
class TestModelInstance:
    ...
    def test_model_instance_creation(self):
        ...
        model = MockModel()
        instance = ModelInstance(model=model, model_type="test", created_at=time.time())
...
class TestAbstractModelManager:
    ...
    def test_gpu_memory_info_pynvml(self):
        ...
        ) as mock_handle, patch("pynvml.nvmlDeviceGetMemoryInfo") as mock_memory:
    ...
    def test_gpu_memory_info_nvidia_smi(self):
        ...
        with patch("pynvml.nvmlInit", side_effect=ImportError), patch(
            "subprocess.run"
        ) as mock_run:

            mock_run.return_value = Mock(
                returncode=0, stdout="16384, 8192, 8192"  # total, free, used in MB
            )
